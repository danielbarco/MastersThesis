...
sys.path.insert(1, 'simrdwn/simrdwn/data_prep/')
import parse_cowc, parse_shapefile
...
def execute(args):
    ...
    for txt_file in listdir(args.annotations_path_root):
        filename, file_extension = os.path.splitext(txt_file)
        print(filename)
        #print(txt_file)
        if isfile(join(args.annotations_path_root, txt_file)) and file_extension == ".txt" and not filename.startswith("._"):
            img = cv2.imread(args.image_path + filename + '.jpg')
            print(img.shape)
            shape = img.shape
            with open(join(args.annotations_path_root, txt_file),'r') as f:
                for line in f:
                    member = line.split()
                    box = [float(member[1]),
                             float(member[2]),
                             float(member[3]),
                             float(member[4])]
                    x0, y0, x1, y1 = from_yolo_to_cor(box, shape)
                    box_coord = [x0, x1, y0, y1]
                    box_coords.append(box_coord)
            category = int(member[0])
            image_path = args.image_path + filename + '.jpg'
            box_coords_matrix = np.matrix(box_coords)            
            print(box_coords, category, image_path)
            gdf = parse_cowc.cowc_box_coords_to_gdf(box_coords_matrix, image_path, category, verbose=False)
            gdf_truth = gdf_truth.append(gdf)
    ...
    #     print(filename)
    #     #print(txt_file)
    #     if isfile(join(args.annotations_path_root, txt_file)) and file_extension == ".xml" and not filename.startswith("._"):
    #         tree = ET.parse(join(args.annotations_path_root, txt_file))
    #         #print(join(path, txt_file))
    #         root = tree.getroot()
    #         for member in root.findall('object'):
    #             x0 = int(member[2][0].text)
    #             y0 = int(member[2][1].text)
    #             x1 = int(member[2][2].text)
    #             y1 = int(member[2][3].text)
    #             box_coord = [x0, x1, y0, y1]
    #             box_coords.append(box_coord)
    ...
    prop_df_file = os.path.join(
        args.prediction_dir, 'test_predictions_aug.csv')
...
def main():
    ...
    # general settings¨
    parser.add_argument('--image_path', type=str, default='simrdwn/data/train_data/VOC2007_SCLD_org/test_images/',
                        help="Location of images")
    parser.add_argument('--annotations_path_root', type=str, default='simrdwn/data/train_data/VOC2007_SCLD_org/labels_corr/',
                        help="Location of XML ground truth labels")
    ...
    #to be deleted
    args.prediction_topdir = 'simrdwn/results/'
